...
import { getAdminDashboardService } from "@/lib/backend/composition/adminDashboardService.server";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const result = await service.getDashboardStats(actor);

    return createBackendResponse(result, actor.requestId);
  } catch (err) {
    console.error("[${publicActor.requestId}] Admin dashboard error", err);
