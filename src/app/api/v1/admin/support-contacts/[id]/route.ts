...
import type { UpdateSupportContactInput } from "@/lib/backend/contracts/support.contract";
...
export async function PATCH(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    let input: UpdateSupportContactInput;
    try {
      input = (await request.json()) as UpdateSupportContactInput;
    } catch {
      return createBackendResponse(fail(validationError("Request body must be valid JSON.")), context.requestId);
    }
    const { id } = await routeContext.params;
    return createBackendResponse(
      await getSupportService().updateContact(id, input, actor.data!),
      context.requestId
    );
  } catch {
...
export async function DELETE(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const { id } = await routeContext.params;
    return createBackendResponse(
      await getSupportService().archiveContact(id, actor.data!),
      context.requestId
    );
  } catch {
    return createBackendResponse(fail(serverError()), context.requestId);
