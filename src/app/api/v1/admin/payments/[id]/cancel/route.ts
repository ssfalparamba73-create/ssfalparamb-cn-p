...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await service.cancelPayment({ paymentId: id, reason: body.reason || 'Cancelled by admin' }, actor);

    return createBackendResponse(result, actor.requestId);
  } catch (err) {
    console.error("[${publicActor.requestId}] Cancel payment error", err);
