      ...
      <div className="flex h-16 items-center justify-between px-4 md:px-8 w-full relative">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <button
            ...
            className="relative size-10 md:size-11 rounded-full bg-[#F8FAFC] border border-[#E2E8F0] shadow-[0_4px_12px_rgba(15,23,42,0.08)] text-slate-400 flex items-center justify-center shrink-0 cursor-not-allowed opacity-70 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-500"
          >
            <Bell className="size-4.5 md:size-5" />
            <span className="absolute top-2 md:top-2.5 right-2 md:right-2.5 size-2 md:size-2.5 rounded-full bg-slate-300 ring-2 ring-white dark:bg-slate-600 dark:ring-slate-800"></span>
          </button>
          ...
          <div className="hidden md:flex flex-col text-right">
            <span className="text-sm font-bold text-slate-900 leading-none mb-1 transition-colors duration-300 dark:text-slate-50">{memberIdentity.name}</span>
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <span className="text-[13px] font-bold text-blue-700 dark:text-blue-300">{memberIdentity.initials}</span>
            </div>
              ...
              {isMenuOpen && (
                ...
                >
                  <div className="px-3 py-3 text-sm text-slate-500 font-medium border-b border-slate-100 mb-2 dark:border-slate-700 dark:text-slate-400">
                    ...
                    <p className="text-slate-800 font-bold text-base truncate dark:text-slate-100">{memberIdentity.name}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
