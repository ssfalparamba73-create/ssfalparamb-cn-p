-- One-time unit bootstrap helper. The follow-up migration removes this RPC after
-- the unit owner has been provisioned. No phone number or PIN is stored here.

CREATE OR REPLACE FUNCTION provision_unit_super_admin(
  p_phone TEXT,
  p_pin TEXT,
  p_name TEXT DEFAULT 'Super Admin'
)
RETURNS TABLE (admin_id UUID, was_created BOOLEAN)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  RETURN QUERY SELECT v_admin.id, v_was_created;
END;
$$;
