-- Replace the single employment/study status with structured, conditional profile fields.
-- The legacy occupation_status column is intentionally retained for rollback compatibility.
...
ALTER TABLE members
  ...
  ADD COLUMN IF NOT EXISTS is_muthaallim BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS muthaallim_institution TEXT,
  ADD COLUMN IF NOT EXISTS work_location TEXT;

ALTER TABLE members DROP CONSTRAINT IF EXISTS members_work_location_check;
ALTER TABLE members ADD CONSTRAINT members_work_location_check
  CHECK (work_location IS NULL OR work_location IN ('india', 'abroad'));

ALTER TABLE members DROP CONSTRAINT IF EXISTS members_student_details_check;
ALTER TABLE members ADD CONSTRAINT members_student_details_check CHECK (
  (NOT is_student AND student_class IS NULL AND student_course IS NULL AND student_institution IS NULL)
  OR
  (is_student
    AND NULLIF(btrim(student_class), '') IS NOT NULL
    AND NULLIF(btrim(student_course), '') IS NOT NULL
    AND NULLIF(btrim(student_institution), '') IS NOT NULL)
);

ALTER TABLE members DROP CONSTRAINT IF EXISTS members_muthaallim_details_check;
ALTER TABLE members ADD CONSTRAINT members_muthaallim_details_check CHECK (
  (NOT is_muthaallim AND muthaallim_institution IS NULL)
  OR
  (is_muthaallim AND NULLIF(btrim(muthaallim_institution), '') IS NOT NULL)
);

CREATE INDEX IF NOT EXISTS idx_members_work_location ON members (work_location);
CREATE INDEX IF NOT EXISTS idx_members_is_student ON members (is_student) WHERE is_student;
CREATE INDEX IF NOT EXISTS idx_members_is_muthaallim ON members (is_muthaallim) WHERE is_muthaallim;
...
BEGIN
  ...
  INSERT INTO members (
    ...
    NULLIF(p_input->>'area', ''),
    NULLIF(btrim(p_input->>'occupation'), ''),
...
BEGIN
  ...
  SET
    ...
    occupation = CASE WHEN p_input ? 'occupation' THEN NULLIF(btrim(p_input->>'occupation'), '') ELSE occupation END,
    ...
    work_location = CASE WHEN p_input ? 'workLocation' THEN NULLIF(p_input->>'workLocation', '') ELSE work_location END,
...
BEGIN
  ...
  END IF;
  IF v_work_location IS NULL OR v_work_location NOT IN ('india', 'abroad') THEN
    RAISE EXCEPTION 'India or Abroad work location is required' USING ERRCODE = '22023';
  END IF;
...
BEGIN
  ...
  IF v_before.profile_completed_at IS NOT NULL AND (
    ...
    NULLIF(btrim(COALESCE(v_after.occupation, '')), '') IS NULL OR
    v_after.work_location IS NULL OR
