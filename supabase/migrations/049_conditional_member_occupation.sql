-- Occupation and work location are required only when a member is neither a student nor a Mutha'allim.
...
BEGIN
  ...
  END IF;
  IF NOT v_is_student AND NOT v_is_muthaallim THEN
...
BEGIN
  ...
  IF v_before.profile_completed_at IS NOT NULL AND (
    ...
    (NOT v_after.is_student AND NOT v_after.is_muthaallim AND (
      NULLIF(btrim(COALESCE(v_after.occupation, '')), '') IS NULL OR
      v_after.work_location IS NULL
    )) OR
